...
template <int N> Coord<N>::Coord(const int init) {
  for (int i = 0; i < N; ++i)
    m_coord[i] = init;
}

template <int N> Coord<N> &Coord<N>::operator=(const Coord<N> &rhs) {
...
template <int N> bool Coord<N>::operator!=(const Coord<N> &rhs) const {
  for (int i = 0; i < N; ++i)
...
template <int N> const uint16_t &Coord<N>::operator[](const uint16_t n) const {
  return m_coord[n];
}

//! Return the value on the \a n - dimension
template <int N> uint16_t &Coord<N>::operator[](const uint16_t n) {
  return m_coord[n];
}
 ...
 */
template <int N> Coord<N> Coord<N>::neigh(int n) const {
  int i = 0;
 ...
 */
template <int N> Coord<N> Coord<N>::parent(int n) const {
  int i = 0;
