...
template <int N> unsigned int Coord<N>::get_sum() const {
  int sum = 0;
  for (int i = 0; i < N; ++i)
    sum += m_coord[i];
  return sum;
...
template <int N> unsigned int Coord<N>::get_part_sum() const {
...
template <int N> unsigned int Coord<N>::get_shift(const int size) const {
  switch (hash_shift) {
  case 0:
...
template <int N> const char *Coord<N>::get_hash_name() {
  return hash_name_pretty[hash_type];
}

template <int N> int Coord<N>::get_hash_shift() { return hash_shift; }

//! Configure \a shift and \a type as hash parameter
template <int N> void Coord<N>::configure_hash(hashType type, int shift) {
  hash_type = type;
  hash_shift = shift;
}
