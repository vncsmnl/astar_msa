...
template <int N> void PAStar<N>::configure_thread_map() {
  map_size =
      m_options.hybrid_conf.p_cores_num * m_options.hybrid_conf.p_cores_size +
  ...
  if (map_size != cont) {
    std::cerr << "BUG on Hybrid conf: " << map_size << " not equal to " << cont
              << std::endl;
    exit(1);
  }
  return;
...
template <int N> void PAStar<N>::enqueue(int tid, std::vector<Node<N>> &nodes) {
  ...
  for (typename std::vector<Node<N>>::iterator it = nodes.begin();
    ...
    if (should_log) {
      ...
      out_stream << tid << "\t" << iter << "\tAdding:\t" << it->pos << "\tg("
                 << it->get_g() << ") h(" << it->get_h() << ") f("
                 << it->get_f() << ")\n";
    }
...
template <int N> void PAStar<N>::wake_all_queue() {
...
void PAStar<N>::worker_inner(int tid, const Coord<N> &coord_final) {
  ...
  while (end_cond == false) {
    ...
    if ((c_search = ClosedList[tid].find(current.pos)) !=
        ClosedList[tid].end()) {
    ...
    for (int i = 0; i < m_options.threads_num; i++) {
      if (i == tid)
        ...
        if (queue_lock.try_lock()) {
          queue_nodes[i].insert(queue_nodes[i].end(), neigh[i].begin(),
                                neigh[i].end());
          queue_condition[i].notify_one();
        } else {
 ...
 */
template <int N> void PAStar<N>::process_final_node(int tid, const Node<N> &n) {
  ...
  if (n.pos.get_id(map_size, thread_map) == (unsigned int)tid) {
    ...
    // std::cout << "[" << tid << "] Agreed with possible answer! " << n << "/"
    // << final_node << std::endl; if (n != final_node) std::cout << "BUG
    // HERE!\n";
    final_node_lock.unlock();
  }
...
#define PASTAR_DECLARE_TEMPLATE(X) template class PAStar<X>;
