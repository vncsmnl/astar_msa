...
template <int N> Coord<N> Sequences::get_final_coord() {
  Coord<N> c;
  for (int i = 0; i < N; ++i)
    c[i] = final_coord[i];
  return c;
...
template <int N> Coord<N> Sequences::get_initial_coord() {
  Coord<N> c;
  return c;
}

//! Create a initial node of the alignment
template <int N> Node<N> Sequences::get_initial_node() {
...
template <int N> Node<N> Sequences::get_final_node() {
  int coord_negative = (1 << N) - 1;
  return Node<N>(0, Sequences::get_final_coord<N>(), coord_negative,
                 SearchDirection::BACKWARD);
}

//! Check if \a c is the destination
template <int N> bool Sequences::is_final(const Coord<N> &c) {
  for (int i = 0; i < N; ++i)
