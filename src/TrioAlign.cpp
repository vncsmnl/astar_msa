...
void TrioAlign::trioCost(int i, int j, int k, const std::string &s1,
  ...
  int c3 =
      at(i + 1, j + 1, k) + Cost::cost(s1.at(i), s2.at(j)) + 2 * Cost::GapCost;
  min_value = std::min(min_value, c3);

  // 5. Advance in s1 and s3 (gap in s2)
  int c4 =
      at(i + 1, j, k + 1) + Cost::cost(s1.at(i), s3.at(k)) + 2 * Cost::GapCost;
  min_value = std::min(min_value, c4);

  // 6. Advance in s2 and s3 (gap in s1)
  int c5 =
      at(i, j + 1, k + 1) + Cost::cost(s2.at(j), s3.at(k)) + 2 * Cost::GapCost;
  min_value = std::min(min_value, c5);

  // 7. Advance in all 3 sequences
  int c6 = at(i + 1, j + 1, k + 1) + Cost::cost(s1.at(i), s2.at(j)) +
           Cost::cost(s1.at(i), s3.at(k)) + Cost::cost(s2.at(j), s3.at(k));
