 ...
 */
void TrioCostSIMDRowImpl(int* HWY_RESTRICT matrix,
                         int /*s1_l*/, int s2_l, int s3_l,
                         int i, int j,
                         char s1_char, char s2_char,
                         const char* HWY_RESTRICT s3,
                         const int* HWY_RESTRICT cost_lut,
                         int gap_cost)
{
    ...
    const int* base_i1_j1  = matrix + idx3D(i + 1, j + 1, 0, s2_l, s3_l); // (i+1, j+1, *)
    ...
    for (; k + static_cast<int>(N) <= s3_l; k += static_cast<int>(N))
    {
        ...
        auto v_c3 = hn::Add(hn::Add(hn::LoadU(d, base_i1_j1 + k), v_cost_s1_s2), v_two_gap);
        ...
        {
            cost_s1_s3_arr[lane] = cost_lut[(int)s1_char * 128 + (int)(unsigned char)s3[k + lane]];
        }
        ...
        for (size_t lane = 0; lane < N; ++lane)
        {
            cost_s2_s3_arr[lane] = cost_lut[(int)s2_char * 128 + (int)(unsigned char)s3[k + lane]];
        }
        ...
        auto v_c6 = hn::Add(hn::Add(hn::Add(v_c6_base, v_cost_s1_s2), v_cost_s1_s3), v_cost_s2_s3);
    ...
    // at(i, j, s3_l) is already initialized (boundary), so we sweep from s3_l-1 down to 0.
    for (k = s3_l - 1; k >= 0; --k)
    {
...
void CallTrioCostSIMDRow(int* matrix,
                         int s1_l, int s2_l, int s3_l,
                         int i, int j,
                         char s1_char, char s2_char,
                         const char* s3,
                         ...
                         int gap_cost)
{
    HWY_DYNAMIC_DISPATCH(TrioCostSIMDRowImpl)(
        matrix, s1_l, s2_l, s3_l,
        i, j, s1_char, s2_char,
        s3, cost_lut, gap_cost);
}
