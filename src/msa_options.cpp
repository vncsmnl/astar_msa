...
int msa_options_core(msa_option_type type, int argc, char *argv[],
                     std::string &filename, PAStarOpt &opt) {
  const std::string description =
      "Usage " + std::string(argv[0]) + " [OPTIONS] file.fasta";
  ...
  common_options.add_options()("version,v", "print version string")(
      "help,h", "produce help message")(
      "cost_type,c",
      po::value<std::string>(&cost_read)->default_value("PAM250"),
      "Match and mismatches costs (NUC for nucleotides, PAM250 for proteins) "
      "[NUC|PAM250]")(
  ...
  parallel_options.add_options()(
      ...
      "number of threads")("affinity,a", po::value<std::string>(&affinity_read),
                           "Comma-separated values to set the affinity of each "
                           "thread, e.g. 0,1,2,3")(
      "hybrid-conf", po::value<std::string>(&hybrid_read),
      "4 Comma-separated values containing: Power Cores Number, Power Cores "
      "thread_map size, "
      "Energy Cores Numbers, Energy Cores thread_map size, e.g. 8,9,8,7")(
      "no-affinity",
      "Do not set thread affinity (force to ignore the affinity argument)")(
      "hash_shift,s",
  ...
  if (vm.count("affinity")) {
    ...
    if (opt.thread_affinity.size() < t_num)
      std::cerr << "Warning: thread affinity set smaller than the number of "
                   "threads. Setting "
                   "default values."
                << std::endl;
  ...
  if (vm.count("hybrid-conf")) {
    ...
    if (opt.threads_num !=
        opt.hybrid_conf.p_cores_num + opt.hybrid_conf.e_cores_num) {
      std::cerr << "Invalid Hybrid conf: P_cores_num + E_cores_num must be "
                   "equal to threads_num"
                << std::endl;
      exit(1);
    }
  } else {
...
int msa_pastar_options(int argc, char *argv[], std::string &filename,
                       PAStarOpt &opt) {
...
//! Parse the arguments
int msa_options(msa_option_type type, int argc, char *argv[],
                std::string &filename, PAStarOpt &opt) {
  try {
    ...
    std::cerr << "Invalid argument: " << e.what()
              << " from option: " << e.get_option_name() << std::endl;
  }
#endif
