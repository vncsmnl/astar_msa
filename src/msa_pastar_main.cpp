...
#define RUN_PASTAR(X)                                                          \
  case X:                                                                      \
    if (opt.dir == SearchDirection::BACKWARD)                                  \
      return PAStar<X>::pa_star(Sequences::get_final_node<X>(),                \
                                Sequences::get_initial_coord<X>(), opt,        \
                                SearchDirection::BACKWARD);                    \
    else                                                                       \
      return PAStar<X>::pa_star(Sequences::get_initial_node<X>(),              \
                                Sequences::get_final_coord<X>(), opt,          \
  ...
  default:
    std::cerr << "Fatal error: Invalid number of sequences: "
              << Sequences::get_seq_num() << std::endl;
  }
  return -1;
}
