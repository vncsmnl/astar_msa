...
void test_brackets() {
  ...
  for (int i = 0; i < 3; ++i) {
    if (c1[i] == c2[i])
      cout << "Bug at: " << c2 << "/" << c1 << ". c1[i] == c2[i], where i=" << i
           << endl;
  }
...
void test_all_bit_hash_zorder() {
  ...
  for (unsigned int i = 1; hash_not_zero; i <<= 1) {
    for (unsigned int j = 0; j < DIM && hash_not_zero; ++j) {
      ...
      cout << "Coordinate:" << c << " curve: " << hash_not_zero << " ("
           << c.z_order_hash_slow<HASH_SHIFT>(THREADS) << '/'
           << c.z_order_hash<HASH_SHIFT>(THREADS) << ')' << endl;
      if (c.z_order_hash_slow<HASH_SHIFT>(THREADS) !=
          c.z_order_hash<HASH_SHIFT>(THREADS)) {
