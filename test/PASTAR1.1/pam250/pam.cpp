...
char order[] = {'C', 'S', 'T', 'P', 'A', 'G', 'N', 'D', 'E', 'Q',
                'H', 'R', 'K', 'M', 'I', 'L', 'V', 'F', 'Y', 'W'};
...
int main(void) {
  ...
  for (unsigned int j = 0; j < sizeof(order); j++) {
    for (unsigned int i = 0; i < sizeof(order); i++)
      if (Cost::cost(order[i], order[j]) !=
          localpam[(int)order[i]][(int)order[j]])
        cout << "Error: " << order[i] << "/" << order[j] << " = "
             << localpam[(int)order[i]][(int)order[j]] << endl;
  }
  cout << "done!\n";
